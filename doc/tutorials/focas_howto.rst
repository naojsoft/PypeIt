...
This doc goes through a full run of PypeIt on one of the Subaru Focas *B*
...
Place all of the files in a single folder. For example,
``/home/username/2024_Pypeit_for_FOCAS/first_reduction/RAW_DATA``
(which will be refer to as ``RAW_PATH``) and the files within are:

.. code-block:: bash

    $ ls
    FCSA00238747.fits  FCSA00238847.fits  FCSA00238849.fits  FCSA00239141.fits
    FCSA00238748.fits  FCSA00238848.fits  FCSA00238850.fits  FCSA00239142.fits
...
Here is the call for these data:

.. code-block:: bash

    cd folder_for_reducing   # this is usually *not* the raw data folder
    pypeit_setup -r ${RAW_PATH}/b -s subaru_focas -c all

This creates a :doc:`../pypeit_file` in the folder named ``subaru_focas_B``
beneath where the script was run.  Note that ``$RAW_PATH`` should be the *full*
path (i.e., as given in the example above).

The ``subaru_focas_B.pypeit`` files looks like this:

.. include:: ../include/subaru_focas_B.pypeit.rst

For some instruments, it is common for frametypes to be
...
.. code-block:: bash
    ...
    cd subaru_focas_b
    run_pypeit subaru_focas_b.pypeit
...
example, which used the standard long-slit of the FOCAS instrument, there is only
one slit.
...
.. code-block:: bash
    ...
    pypeit_chk_edges Calibrations/Edges_B_1_DET01.fits.gz

.. figure:: ../figures/focasb_edges_image.png
   :width: 40%

The data is the combined flat images and the green/magenta
lines indicate the left/right slit edges.  The S457 label
...
The PNGs can also be viewed through the link.
...
the ``QA/PNGs/Arc_1dfit_B_1_DET01_S0457`` file:

.. figure:: ../figures/focasb_arc1d.png
   :width: 90%
...
Here is ``QA/PNGs/Arc_tilts_2d_B_1_DET01_S0457.png``:

.. figure:: ../figures/focasb_arc2d.png
   :width: 50%
...
.. code-block:: bash
    ...
    pypeit_chk_tilts Calibrations/Tilts_B_1_DET01.fits

.. figure:: ../figures/focasb_ginga_tilts.png
   :width: 40%
...
.. code-block:: bash
    ...
    pypeit_chk_flats Calibrations/Flat_B_1_DET01.fits

.. figure:: ../figures/focasb_flat.png
   :width: 40%
...
.. code-block:: bash
    ...
    pypeit_show_2dspec Science/spec2d_FCSA00238848-GD71_FOCAS_20230111T102519.981.fits

.. figure:: ../figures/focasb_spec2d.png
   :width: 40%
...
.. code-block:: bash
    ...
    pypeit_show_1dspec Science/spec1d_FCSA00238848-GD71_FOCAS_20230111T102519.981.fits

.. figure:: ../figures/focasb_spec1d.png
   :width: 70%
...
.. code-block:: bash
    ...
    pypeit_sensfunc Science/spec1d_FCSA00238848-GD71_FOCAS_20230111T102519.981.fits

This produces the sensitivity function
and three QA (pdf) plots.  The main QA plot looks like this:

.. figure:: ../figures/focasb_sens.png
   :width: 60%
